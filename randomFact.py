...
"disgruntled employee", "great VV", "step-VV", "friend", "homie", "pal", "sugar momma", "sugar daddy", "baby daddy"]
...
def returnFact():
    ...
    or fact.find("!!") != -1 or fact.find("??") != -1):
        ...
        if fact.find("??") != -1:
            if fact.find("??CC") != -1:
                fact = fact.replace("??CC", getRand(interjection).capitalize(), 1)
            else:
                fact = fact.replace("??", getRand(interjection), 1)
